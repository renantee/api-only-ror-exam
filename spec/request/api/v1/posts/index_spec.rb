...
def post_meta
  @links = { first: "http://127.0.0.1:8000/api/posts?page=1",
...
def expected_post_index(post)
  {
    data: [{
      "id"         => post.id, "user_id" => post.user_id, "title" => post.title,
      "slug"       => post.slug, "image" => post.image, "content" => post.content,
...
RSpec.describe "Posts", type: :request do
  ...
  describe "GET /api/posts#index" do
    ...
    it "returns response match to expected json" do
      post_meta
      expect(response.body).to be_json_as(expected_post_index(@post))
    end
  end
end
